...
int PALIAL_dtsqrt (int tileA_size, double *tileA1, int lda1, double *tileA2, int lda2, double *tileS, int lds, double *tho, double *workspace)
{
    double alpha;
    int lambda, gamma;
    if (tileA_size == 0)
    ...
    for (lambda = 0; lambda < tileA_size; lambda += tileA_size)
    {
        ...
        for (int i = 0; i < gamma; i++)
        {
            LAPACKE_dlarfg_work(tileA_size + 1,
                    ...
                    &tileA2[lda2 * (lambda + i)], 1,
                    &tho[lambda + i]);
            if (lambda + i + 1 < tileA_size)
            {
                alpha = -tho[lambda + i];
                cblas_dcopy(gamma - i - 1,
                        &tileA1[lda1 * (lambda + i + 1) + (lambda + i)],
                        lda1,
                        workspace,
                        1);
                cblas_dgemv(CblasColMajor,
                        ...
                        &tileA2[lda2 * (lambda + i + 1)],
                        lda2,
                        &tileA2[lda2 * (lambda + i)],
                        1,
                        1.0,
                        workspace,
                        1);
                cblas_daxpy(gamma - i - 1,
                        alpha,
                        workspace,
                        1, 
                        &tileA1[lda1 * (lambda + i + 1) + lambda + i],
        ...
        if (tileA_size > lambda + gamma)
        {
            PALIAL_dtsmqr(PALIAL_left,
                    PALIAL_transpose,
                    gamma,
                    tileA_size - (lambda + gamma),
                    tileA_size,
                    tileA_size,
                    &tileA1[lda1 * (lambda + gamma) + lambda],
                    lda1,
                    &tileA2[lda2 * (lambda + gamma)],
                    lda2,
                    &tileA2[lda2 * lambda],
                    lda2,
                    &tileS[lds * lambda],
